...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetAgentRevisionRequest$outboundSchema, value),
  ...
  const pathParams = {
    ...
    revision: encodeSimple("revision", payload.revision, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/agents/{name}/revisions/{revision}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.AgentRevision$inboundSchema),
