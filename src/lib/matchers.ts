...
import { AlbusDefaultError } from "../models/errors/albus-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AlbusDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AlbusDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AlbusDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AlbusDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
